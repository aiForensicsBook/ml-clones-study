...
\thanks{Replication package: \url{https://github.com/aiForensicsBook/ml-clones-study}}}
\subtitle{Measuring Accidental Clone Prevalence in PyTorch Projects}

\titlerunning{Framework-Dictated ML Code Similarity}

\author{Joseph C.\ Sremack}

\authorrunning{J.\ C.\ Sremack}

\institute{J.\ C.\ Sremack \at
              \email{joe.sremack@cbiz.com}}
...
I present an empirical study of 15 verified-independent PyTorch projects spanning five ML domains (computer vision, NLP, audio, generative modeling, and other). I develop an AST-based classifier that categorizes every function and class as framework boilerplate, API protocol code, or custom implementation, validated against PyTorch's official documentation and through intra-rater reliability assessment. I apply JPlag clone detection to all pairwise project combinations before and after filtering framework-dictated code, measuring the \emph{filtration effect}, the change in similarity attributable to shared boilerplate. Sensitivity analyses across multiple classification and detection thresholds assess the robustness of findings. This study contributes the first empirical baseline for accidental clone prevalence in ML framework code and a reusable methodology for boilerplate-aware similarity measurement.
\keywords{Code clones \and Boilerplate code \and Machine learning \and PyTorch \and Code similarity \and Software forensics}
\end{abstract}
...
The remainder of this paper is organized as follows. Section~\ref{sec:related} reviews related work. Section~\ref{sec:method} describes my study design. Section~\ref{sec:results} presents results. Section~\ref{sec:discussion} discusses findings and implications. Section~\ref{sec:threats} addresses threats to validity. Section~\ref{sec:conclusion} concludes.
...
Kapser and Godfrey~\citep{kapser2008} developed a taxonomy of clone patterns that includes boilerplate clones, API protocol clones, and language idioms, arguing that many clones are unavoidable or even beneficial. Their taxonomy provides the conceptual foundation for my classification scheme: my Category~A (framework boilerplate) corresponds to their boilerplate pattern, Category~B (API protocol) to their API protocol pattern, and Category~C (custom implementation) to code that would not appear as a clone pattern in their taxonomy.

Ragkhitwetsagul, Krinke, and Clark~\citep{ragkhitwetsagul2018} tested 30 code similarity analysis tools across multiple scenarios, including a boilerplate injection scenario. They demonstrated that boilerplate code confounds similarity tools by inflating measured similarity, but they did not measure boilerplate prevalence in real-world ML projects. My study fills this gap by measuring both prevalence and its effect on similarity scores.

\subsection{ML Software Engineering}

Machine learning software has distinctive characteristics that affect code similarity analysis. Sens et al.~\citep{sens2025} analyzed the structure of 2,928 ML systems, finding that 58\% contain duplicated model implementation files. Their work documents the prevalence of duplication but does not distinguish framework-dictated duplication from meaningful reuse. Nahar et al.~\citep{nahar2023} studied 262 ML products and found that independently developed products converge on shared architectural patterns. Dilhara et al.~\citep{dilhara2022} mined 1,000 top-rated Python ML repositories and discovered 28,308 repetitive code change patterns, demonstrating that ML code contains pervasive structural repetition at the change level as well. This convergence is consistent with the framework effect I hypothesize: when developers use the same ML framework, their code necessarily shares structural patterns.

\subsection{Gap}

Prior work establishes that (1)~accidental clones exist and arise from shared constraints, but this was measured without a shared framework~\citep{juergens2010}; (2)~boilerplate and API protocol clones are recognized categories~\citep{kapser2008}, but their prevalence in ML code is unknown; (3)~boilerplate confounds similarity tools~\citep{ragkhitwetsagul2018}, but the magnitude of this confound in ML projects has not been measured; and (4)~ML code exhibits high duplication~\citep{sens2025}, structural convergence~\citep{nahar2023}, and repetitive code change patterns~\citep{dilhara2022}, but the framework-driven component has not been isolated. My study provides the first empirical measurement of framework-driven code similarity in ML projects and quantifies the effect of filtering it out.
...
\centering
\begin{tikzpicture}[
    phase/.style={draw, rounded corners, fill=blue!8, text width=3.2cm,
                  minimum height=2.4cm, align=center, font=\small},
    arr/.style={-{Stealth[length=3mm]}, thick, draw=black!60},
    label/.style={font=\scriptsize, text=black!70, align=center}
]
\node[phase] (p1) {\textbf{Phase 1}\\[2pt]Data Collection\\[4pt]\scriptsize GitHub API mining\\15 projects selected\\Independence verified};
\node[phase, right=0.5cm of p1] (p2) {\textbf{Phase 2}\\[2pt]AST Classification\\[4pt]\scriptsize Python \texttt{ast} module\\A/B/C categories\\JSON manifests};
\node[phase, right=0.5cm of p2] (p3) {\textbf{Phase 3}\\[2pt]Similarity\\Measurement\\[4pt]\scriptsize JPlag pairwise\\Full + filtered code};
\node[phase, right=0.5cm of p3] (p4) {\textbf{Phase 4}\\[2pt]Analysis\\[4pt]\scriptsize Filtration effect\\Sensitivity analysis\\Statistical tests};
\draw[arr] (p1) -- (p2);
\draw[arr] (p2) -- (p3);
\draw[arr] (p3) -- (p4);
\end{tikzpicture}
...
The validity of the study depends on the selected projects being truly independent. For every pair of selected projects, I verified: (1)~no shared contributors, (2)~no fork relationships, (3)~no shared organizational affiliation, (4)~different project creation dates, and (5)~no evidence of seeding from a shared template (via manual inspection of earliest commits). An automated script checked contributor overlap and fork networks via the GitHub API, with manual verification of flagged pairs. The complete independence verification matrix is included in the replication package.
...
\textbf{Framework Documentation Ground Truth.} For every code unit classified as Category~A, I check whether the pattern appears in PyTorch's official tutorials or examples repository. This provides an objective precision metric: what percentage of Category~A classifications have a documented pattern match?

\textbf{Intra-Rater Reliability.} A stratified random sample of 50 code units (16 Category~A, 11 Category~B, 23 Category~C, proportional to the population with oversampling of medium/low-confidence classifications) was manually classified by the author using a detailed codebook (included in the replication package). After a washout period, the same 50 code units were re-classified in shuffled order without access to prior labels. Cohen's $\kappa$~\citep{cohen1960} between Round~1 and Round~2 measures classification consistency. The target is $\kappa \geq 0.70$ (substantial agreement per Landis and Koch's~\citep{landis1977} benchmark scale). This measures consistency, not objectivity, a limitation I acknowledge explicitly.
...
\subsubsection{Tool.}
I use JPlag v5.1.0~\citep{jplag2000,saglam2024}, a token-based code similarity detector supporting Python~3. JPlag compares all submissions pairwise and reports a similarity percentage for each pair. I chose JPlag for its open-source licensing (Apache 2.0), mature Python support, and established position in the similarity analysis literature. Ragkhitwetsagul et al.~\citep{ragkhitwetsagul2018} compared JPlag with NiCad and 28 other tools, providing published evidence for its capabilities and limitations.
...
Table~\ref{tab:demographics} summarizes the 15 selected projects. The dataset spans five ML domains and ranges from 497 to 23,742 lines of code, totaling 88,032 LOC across 583 files and 3,488 classifiable code units.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{fig1_classification_by_domain.pdf}
...
\emph{Summary:} Table~\ref{tab:similarity} summarizes the pairwise similarity distributions. The mean pairwise similarity between independent projects using full code was 5.34\%, substantially above the $<$1\% baseline reported by Juergens et al.~\citep{juergens2010} for implementations without a shared framework. Within-domain pairs showed slightly lower similarity (5.19\%) than cross-domain pairs (5.38\%), though this difference was not statistically significant (Mann-Whitney $U = 901.5$, $p = 0.88$, Cliff's $\delta = 0.02$).
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\textwidth]{fig2_boxplot_filtration.pdf}
...
\emph{Summary:} Figure~\ref{fig:sensitivity} and Table~\ref{tab:sensitivity} present the sensitivity results. The boilerplate proportion (A+B as percentage of code removed) varied by only 6.9 percentage points across the five classification thresholds tested (range: 51.7--58.6\%). The filtration effect was similarly stable, varying by only 0.07 percentage points (range: 2.05--2.12pp). This stability indicates that the core findings are robust to the specific classification threshold chosen.

\subsection{Validation Results}

\subsubsection{Intra-Rater Reliability.}
The intra-rater validation yielded Cohen's $\kappa = 0.35$ (fair agreement) and linearly weighted $\kappa_w = 0.46$ (moderate agreement) across 50 stratified code units classified in two rounds separated by a washout period. Raw agreement was 58.0\% (29/50).

Agreement varied substantially by category. Category~A (framework boilerplate) achieved 61.1\% agreement (11/18), Category~C (custom implementation) achieved 84.2\% (16/19), but Category~B (API protocol) achieved only 15.4\% (2/13). The dominant disagreement pattern was B$\rightarrow$C reclassification (9 of 21 disagreements): code units classified as API protocol in Round~1 were reclassified as custom implementation in Round~2.

This pattern is interpretable. The B/C boundary is inherently ambiguous: code that follows framework conventions but incorporates domain-specific logic can reasonably be classified either way. Collapsing to a binary scheme (A~vs.\ non-A) yielded $\kappa = 0.54$ (moderate agreement) with 80.0\% raw agreement (40/50), confirming that the boilerplate boundary is substantially more stable than the three-category scheme. The A~vs.\ non-A boundary is more stable because Category~A patterns (training loops, checkpoint save/load, device management) have unambiguous structural signatures. The low overall $\kappa$ thus reflects genuine boundary ambiguity at the B/C boundary rather than arbitrary inconsistency, and it is precisely this ambiguity that motivates the sensitivity analysis.

\subsubsection{Sensitivity as Complementary Validation.}
The sensitivity analysis (Section~\ref{sec:results}, RQ4) provides the critical complementary evidence. Because the B/C boundary shifts with the classification threshold, and the intra-rater results confirm that this boundary is subjective, the stability of core findings across all five thresholds (filtration effect range: 2.05--2.12pp, coefficient of variation $<$5\%) demonstrates that the study's conclusions do not depend on resolving the B/C ambiguity. Whether borderline code units are classified as B or C, the measured framework effect remains essentially unchanged.
...
Juergens et al.~\citep{juergens2010} reported $<$1\% clone detection rates for 109 independent implementations of the same specification without a shared framework. My results show that when a shared framework is introduced, baseline similarity increases to 5.34\%, more than five times higher. This confirms the hypothesis that framework constraints fundamentally change the interpretation of code similarity measurements.

Sens et al.~\citep{sens2025} found 58\% duplication in ML systems. My measurements suggest that a large portion of this duplication may be framework-dictated rather than representing meaningful code reuse. The 52.7\% boilerplate proportion I measured provides context for interpreting their finding.

Dilhara et al.~\citep{dilhara2022} discovered 28,308 repetitive code change patterns across 1,000 Python ML repositories, demonstrating that structural repetition pervades ML code not only at the static level but at the change level as well. My results suggest a possible mechanism: if 52.7\% of code units are framework-dictated, then a substantial fraction of code changes will also be framework-dictated (e.g., updating a training loop when migrating PyTorch versions, or modifying a \texttt{Dataset} subclass interface). The boilerplate prevalence I measure may thus partially explain the repetitive change patterns they observed.

\subsection{Why Within-Domain and Cross-Domain Similarity Are Similar}

A counterintuitive finding is that within-domain pairs (5.19\% mean similarity) and cross-domain pairs (5.38\%) show nearly identical baseline similarity ($p = 0.88$, Cliff's $\delta = 0.02$). One might expect that projects in the same ML domain (e.g., two computer vision projects) would share more code than projects in different domains (e.g., a vision project and an audio project), since they are more likely to use similar model architectures, loss functions, and data preprocessing pipelines.

The data suggest that the framework effect dominates domain-specific effects. PyTorch's core API patterns---training loops, module definitions, optimizer configuration, checkpoint management---are shared across all domains. These universal patterns account for the bulk of detected similarity. Domain-specific code (e.g., convolutional architectures in vision, attention mechanisms in NLP) falls predominantly into Category~C (custom implementation), which is filtered out when computing baseline similarity but contributes relatively little to full-code similarity because it is structurally diverse even within a domain. This pattern holds across all three JPlag thresholds: at the sensitive threshold ($t=5$), within-domain pairs average 31.36\% versus 34.18\% for cross-domain; at the strict threshold ($t=25$), 0.22\% versus 0.11\%. The absence of a domain effect at any threshold strengthens the interpretation that framework conformity, not domain similarity, drives detected resemblance.

\subsection{Characteristics of High-Filtration Pairs}

The filtration effect varied substantially across pairs, ranging from $-2.98$ to $+8.94$ percentage points. Examining the pairs with the largest filtration effects reveals what drives them. The top five pairs (Diffusion-Distillation-WL/pytorch-examples, BenchmarkTransferLearning/chatterbox-finetuning, ANTsTorch/Diffusion-Distillation-WL, ANTsTorch/chatterbox-finetuning, and chatterbox-finetuning/pytorch-examples) all involve at least one project with a high boilerplate proportion: Diffusion-Distillation-WL (68.7\% A+B), chatterbox-finetuning (66.6\%), and pytorch-examples (64.0\%). The average combined boilerplate proportion for the top five pairs ranges from 56.8\% to 66.3\%.

The correlation between average boilerplate proportion and filtration effect is moderate ($r = 0.39$), indicating that boilerplate prevalence is an important but not sole predictor. Other factors include how much the boilerplate code specifically overlaps between two projects (two projects may both have high boilerplate but use different boilerplate patterns) and the relative size of the custom codebases after filtering.

Of the 105 pairs, 40 (38.1\%) showed a relative reduction exceeding 50\%, meaning that more than half of their detected similarity was attributable to framework code. Sixteen pairs exceeded 10\% full-code similarity, a threshold that might trigger concern in a plagiarism or forensic context. After filtering, zero pairs remained above 10\%. This illustrates the practical stakes: without boilerplate filtering, a forensic analyst reviewing these projects might flag 16 pairs as warranting investigation, when the framework-aware analysis eliminates all of them.

Negative filtration effects (where filtered similarity exceeds full-code similarity) occurred in 25 pairs (23.8\%). This happens when removing boilerplate code makes the remaining custom code proportionally more similar---a statistical artifact of denominator reduction rather than evidence that boilerplate filtering increases true similarity. These negative cases are generally small in magnitude (median $-1.04$ pp).
...
\emph{Clone detection research.} Studies measuring clone rates in ML code should account for framework boilerplate to avoid inflating reported rates. My classifier and filtering methodology provide a reusable tool for this purpose.
...
\emph{Tool development.} Clone detection and plagiarism detection tools that operate on ML code could incorporate framework-awareness directly. A practical approach would be to ship framework-specific boilerplate profiles (for PyTorch, TensorFlow, etc.) that allow tools to either exclude boilerplate matches or report framework-adjusted similarity scores alongside raw scores. The AST-based classifier developed in this study demonstrates one such approach; a lighter-weight alternative could use pattern matching on import statements and API calls without full AST parsing. The stability of findings across classification thresholds (Section~\ref{sec:results}, RQ4) suggests that even an approximate boilerplate filter would substantially improve the interpretability of similarity scores for ML code.

\subsection{Limitations of the Filtration Effect}

The filtration effect as measured here is a \emph{lower bound} on the true framework contribution to similarity. My binary filtering (remove A+B, keep C) may not capture all framework influence; some Category~C code may still be indirectly shaped by framework conventions (e.g., tensor shape conventions that propagate into custom code). Additionally, my Category~B (API protocol) code occupies a gray area: it follows framework patterns but includes domain-specific content.
...
The primary construct validity concern is the accuracy of the AST-based classifier. The three-category classification scheme necessarily involves boundary cases, particularly for \texttt{nn.Module} subclasses that contain both framework-dictated structure and custom logic. Intra-rater reliability was below the target ($\kappa = 0.35$), with disagreements concentrated at the Category~B/C boundary (9 of 21 disagreements were B$\rightarrow$C reclassifications). I address this through: (1)~validation against PyTorch documentation as objective ground truth, (2)~a detailed codebook with explicit boundary-case rules, and (3)~sensitivity analysis showing that findings are stable across all five classification thresholds despite the B/C boundary ambiguity.

The choice of JPlag (token-based) over NiCad (AST-based) for similarity measurement means my results capture syntactic rather than structural similarity. Token-based comparison is arguably more conservative, which would make positive findings about accidental clones more robust. However, results may differ with AST-based tools, and I identify cross-tool comparison as future work.

\subsection{Internal Validity}

The study uses a solo-researcher validation design. The intra-rater $\kappa$ of 0.35 fell below the 0.70 target, indicating that the three-category classification scheme has limited reproducibility at the B/C boundary. However, the weighted $\kappa$ of 0.46 (moderate agreement) reflects that most disagreements were between adjacent categories rather than between A and C. More importantly, the sensitivity analysis demonstrates that the core findings are invariant to where the B/C boundary falls. A multi-rater validation study with independent software engineers would strengthen construct validity and is planned as future work.

\subsection{External Validity}

My study examines a single framework (PyTorch) in a single language (Python) across a limited number of projects (15). While PyTorch is the most widely used ML research framework, results may not generalize to TensorFlow, JAX, or frameworks in other languages. The specific project selection criteria (500--5{,}000 LOC, $\leq$3 contributors, 30+ commits) target a particular project profile; larger or more complex projects may exhibit different boilerplate proportions.

I focus on five ML domains (computer vision, NLP, audio, generative modeling, and other). Additional domains (reinforcement learning, graph neural networks) may have different framework usage patterns.

\subsection{Conclusion Validity}

The small sample size (15 projects, 105 pairwise comparisons) limits statistical power. I use non-parametric tests appropriate for small samples and report effect sizes alongside $p$-values. Bootstrap confidence intervals provide uncertainty quantification. I frame this as an initial measurement study; larger replications are future work.
...
J.\ C.\ Sremack is the sole author and was responsible for all aspects of this work, including study design, data collection, analysis, and writing.

\subsection*{Data Availability Statement}
All data, code, and analysis scripts are available in the replication package at \url{https://github.com/aiForensicsBook/ml-clones-study}. All repositories used in this study are publicly available on GitHub.
...
\bibitem[Nahar et~al.(2023)]{nahar2023}
Nahar, N., Zhang, H., Lewis, G., Zhou, S., \& K\"{a}stner, C. (2023).
...
\newblock In \emph{Proc.\ 2nd Int.\ Conf.\ on AI Engineering (CAIN)}, pp.\ 171--183.
...
\bibitem[Dilhara et~al.(2022)]{dilhara2022}
Dilhara, M., Ketkar, A., Sannidhi, N., \& Dig, D. (2022).
\newblock Discovering repetitive code changes in {Python} {ML} systems.
\newblock In \emph{Proc.\ 44th Int.\ Conf.\ on Software Engineering (ICSE)}, pp.\ 736--748.

\bibitem[Landis \& Koch(1977)]{landis1977}
...
\newblock In \emph{Advances in Neural Information Processing Systems (NeurIPS)}, 32, 8024--8035.

\bibitem[S\u{a}glam et~al.(2024)]{saglam2024}
S\u{a}glam, T., Br\"{o}del, M., Schmid, L., \& Hahner, S. (2024).
\newblock Obfuscation-resilient software plagiarism detection with {JPlag}.
\newblock In \emph{Proc.\ 46th Int.\ Conf.\ on Software Engineering: Companion (ICSE Companion)}, pp.\ 240--244.
